...
step "Generate Gatling SSL keystore & trustore" do
  ...
  `cat #{ssldir}/mastercert.pem #{ssldir}/masterkey.pem > #{ssldir}/keystore.pem`
  `echo "puppet" | openssl pkcs12 -export -in #{ssldir}/keystore.pem \
   -out #{ssldir}/keystore.p12 -name #{master_certname} -passout fd:0`
  `keytool -importkeystore -destkeystore #{ssldir}/gatling-keystore.jks \
   -srckeystore #{ssldir}/keystore.p12 -srcstoretype PKCS12
   -alias #{master_certname} -deststorepass "puppet" -srcstorepass "puppet"`

  # Generate truststore
  `keytool -import -alias "CA" -keystore #{ssldir}/gatling-truststore.jks \
   -storepass "puppet" -trustcacerts -file #{ssldir}/cacert.pem -noprompt`
end
