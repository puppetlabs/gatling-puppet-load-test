...
step "Set up SSH key for github access" do
  unless jenkins.file_exist?("/root/.ssh/id_rsa")
  ...
  unless jenkins.file_exist?("/root/.ssh/id_rsa.pub")
...
step "add github to known hosts" do
  ...
  create_remote_file(jenkins, "/root/.ssh/known_hosts", <<-KNOWN_HOSTS)
